...
ChannelCoordinator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ChannelCoordinator")
    ...
    .AddAttribute ("SchInterval", "SCH Interval, default value is 50ms.",
                   ...
                   MakeTimeChecker ())
	.AddAttribute ("RMInterval", "RM Interval, default value is 25ms.",
				   TimeValue (GetDefaultRMInterval ()),
				   MakeTimeAccessor (&ChannelCoordinator::m_rm),
				   MakeTimeChecker ())
	.AddAttribute ("CDQInterval", "CDQ Interval, default value is 15ms.",
                   TimeValue (GetDefaultCDQInterval ()),
				   MakeTimeAccessor (&ChannelCoordinator::m_cdq),
				   MakeTimeChecker ())
	.AddAttribute ("AMInterval", "AM Interval, default value is 10ms.",
				   TimeValue (GetDefaultAMInterval ()),
				   MakeTimeAccessor (&ChannelCoordinator::m_am),
				   MakeTimeChecker ())
...
Time
ChannelCoordinator::GetDefaultRMInterval (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  // refer to Annex H of IEEE 1609.4-2010
  const static uint8_t DEFAULT_RM_INTERVAL = 25;
  return MilliSeconds (DEFAULT_RM_INTERVAL);
}
Time
ChannelCoordinator::GetDefaultCDQInterval (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  // refer to Annex H of IEEE 1609.4-2010
  const static uint8_t DEFAULT_CDQ_INTERVAL = 15;
  return MilliSeconds (DEFAULT_CDQ_INTERVAL);
}
Time
ChannelCoordinator::GetDefaultAMInterval (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  // refer to Annex H of IEEE 1609.4-2010
  const static uint8_t DEFAULT_AM_INTERVAL = 10;
  return MilliSeconds (DEFAULT_AM_INTERVAL);
}
...
Time
ChannelCoordinator::GetRMInterval (void) const
{
  NS_LOG_FUNCTION (this);
  return m_rm;
}
...
bool
ChannelCoordinator::IsRMInterval (Time duration) const
{
  NS_LOG_FUNCTION (this << duration);
  Time future = GetIntervalTime (duration);
  return (future < m_rm);
...
bool
ChannelCoordinator::IsCDQInterval (Time duration) const
{
  NS_LOG_FUNCTION (this << duration);
  Time future = GetIntervalTime (duration);
  return (m_rm < future && future < (m_cdq+m_rm));
}

bool
ChannelCoordinator::IsAMInterval (Time duration) const
{
  NS_LOG_FUNCTION (this << duration);
  Time future = GetIntervalTime (duration);
  return ((m_cdq+m_rm)<future && future < m_cchi);
}
