...
WaveEdcaTxopN::NotifyChannelSwitching (void)
{
  ...
  {
	  Ptr<WifiMacQueueItem> wifi_mac_queue_item(new WifiMacQueueItem(m_currentPacket, m_currentHdr));
	  EdcaTxopN::GetQueue()->PushFront (wifi_mac_queue_item);
	  m_currentPacket = 0;
  }
}
...
WaveEdcaTxopN::Queue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  Ptr<WifiMacQueueItem> item(new WifiMacQueueItem(packet, hdr));
  i->second->Enqueue (item);
