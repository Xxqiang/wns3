...
void
MultipleChannelsExperiment::SendRmPackets (double time, uint32_t channelNumber)
{
	NetDeviceContainer::Iterator i;
	for (i = devices.Begin (); i != devices.End (); ++i)
	    {
	      Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice> (*i);
	      Simulator::Schedule(Seconds(rng->GetValue (time, time+0.020)),
...
void
MultipleChannelsExperiment::SendAmPackets (Ptr<WaveNetDevice> sender, uint32_t channelNumber)
{
...
void
MultipleChannelsExperiment::SendSchPackets ()
{
	NetDeviceContainer::Iterator i;
	for (i = devices.Begin (); i != devices.End (); ++i){
		Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice> (*i);
  ...
  while(getCurrentTime()-time_start < simulationTime)
  {
	  ...
	  if(coordinator_flag->IsCchInterval(t)&&flag==0){
		  ...
		  if(coordinator_flag->IsAMInterval(t)){
			  ...
			  NetDeviceContainer::Iterator i;
			  Simulator::Schedule(t,&MultipleChannelsExperiment::AmInit, this);
			  for (i = devices.Begin (); i != devices.End (); ++i){
				  Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice> (*i);
			  	  Simulator::Schedule(Seconds(rng->GetValue (now, now + 0.008)),
