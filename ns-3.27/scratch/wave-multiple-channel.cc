...
public:
	StatsTag(void) :
			m_packetId(0), m_sendTime(Seconds(0)) {
	}
	StatsTag(uint32_t packetId, Time sendTime) :
			m_packetId(packetId), m_sendTime(sendTime) {
	}
...
TypeId StatsTag::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::StatsTag").SetParent<Tag>().AddConstructor<
			StatsTag>();
	return tid;
}
TypeId StatsTag::GetInstanceTypeId(void) const {
	return GetTypeId();
}
uint32_t StatsTag::GetSerializedSize(void) const {
	return sizeof(uint32_t) + sizeof(uint64_t);
}
void StatsTag::Serialize(TagBuffer i) const {
	i.WriteU32(m_packetId);
	i.WriteU64(m_sendTime.GetMicroSeconds());
}
void StatsTag::Deserialize(TagBuffer i) {
	m_packetId = i.ReadU32();
	m_sendTime = MicroSeconds(i.ReadU64());
}
void StatsTag::Print(std::ostream &os) const {
...
private:
	...
	bool Receive(Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t mode,
...
MultipleChannelExperiment::MultipleChannelExperiment(void) :
		...
		frequencySafety(100),       // 10Hz, 100ms send one safety packet
		...
		safetyPacketID(0), nonSafetyPacketID(0), receiveSafety(0), receiveNonSafety(
				0), queueSafety(0), queueNonSafety(0), timeSafety(0), timeNonSafety(
				0), createTraceFile(false) {
}

bool MultipleChannelExperiment::Configure(int argc, char **argv) {
	CommandLine cmd;
	...
	cmd.AddValue("sizeNonSafety", "Size of non-safety packet, bytes.",
			sizeNonSafety);
	cmd.AddValue("frequencySafety", "Frequency of sending safety packets, Hz.",
			frequencySafety);
	cmd.AddValue("frequencyNonSafety",
			"Frequency of sending non-safety packets, Hz.", frequencyNonSafety);
	cmd.AddValue("createTraceFile",
			"create trace file with 4 different configuration",
			createTraceFile);

	cmd.Parse(argc, argv);
	return true;
}
void MultipleChannelExperiment::Usage(void) {
	std::cout << "usage:"
...
double MultipleChannelExperiment::getCurrentTime() {
	struct timeval tv;
	gettimeofday(&tv, NULL);    //该函数在sys/time.h头文件中
	return tv.tv_sec + tv.tv_usec / 1000000.0;
}

void MultipleChannelExperiment::CreateWaveNodes(void) {
	...
	for (uint32_t i = 0; i != devices.GetN(); ++i) {
		devices.Get(i)->SetReceiveCallback(
				MakeCallback(&MultipleChannelExperiment::Receive, this));
	}
}

bool MultipleChannelExperiment::Receive(Ptr<NetDevice> dev,
		Ptr<const Packet> pkt, uint16_t mode, const Address &sender) {
	...
	if (createTraceFile)
		outfile << "Time = " << std::dec << now.GetMicroSeconds()
				<< "us, receive packet: " << " protocol = 0x" << std::hex
				<< mode << std::dec << " id = " << packetId << " sendTime = "
				<< sendTime.GetMicroSeconds() << " type = "
				<< (mode == WSMP_PROT_NUMBER ?
						"SafetyPacket" : "NonSafetyPacket") << std::endl;
	...
	if (i == broadcastPackets.end()) {
		...
		NS_LOG_DEBUG(
				"the node [" << nodeId << " out of source node's transmission range" " receive the packet [" << packetId << "].");
		return true;
	}
...
void MultipleChannelExperiment::SendIpPackets(Ptr<WaveNetDevice> sender) {
...
void MultipleChannelExperiment::SendWsmpPackets(Ptr<WaveNetDevice> sender,
	...
	if (neighbors->size() == 1) {
		NS_LOG_WARN(
				"it may be strange that this node[" << src->GetId() <<"] has no neighbors when it is sending a packet[" << tag.GetPacketId() << "].");
	}
	...
	if (createTraceFile) {
		if (result)
			outfile << "Time = " << now.GetMicroSeconds()
					<< "us, broadcast WSMP packet: ID = " << tag.GetPacketId()
					<< std::endl;
		else
...
void MultipleChannelExperiment::InitStats(void) {
	...
	Simulator::ScheduleDestroy(&MultipleChannelExperiment::StatQueuedPackets,
			this);
}
/*
 void
...
void MultipleChannelExperiment::StatQueuedPackets() {
	NS_LOG_FUNCTION(this);
	NetDeviceContainer::Iterator i;
	for (i = devices.Begin(); i != devices.End(); ++i) {
		Ptr<WaveNetDevice> device = DynamicCast<WaveNetDevice>(*i);
		Ptr<RegularWifiMac> rmac = DynamicCast<RegularWifiMac>(
				device->GetMac(178));
...
void MultipleChannelExperiment::ConfigurationA(void) {
	NS_LOG_FUNCTION(this);
	NetDeviceContainer::Iterator i;
	for (i = devices.Begin(); i != devices.End(); ++i) {
		Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice>(*i);
		SchInfo schInfo = SchInfo(SCH1, false, 0xff);
		Simulator::Schedule(Seconds(0.0), &WaveNetDevice::StartSch, sender,
				schInfo);
		TxProfile profile = TxProfile(SCH1);
		Simulator::Schedule(Seconds(0.0), &WaveNetDevice::RegisterTxProfile,
				sender, profile);
		for (uint32_t time = 0; time != simulationTime; ++time) {
			for (uint32_t sends = 0; sends != frequencySafety; ++sends) {
				Simulator::Schedule(
						Seconds(rngSafety->GetValue(time, time + 1)),
						&MultipleChannelExperiment::SendWsmpPackets, this,
						sender, SCH1);
			}
			for (uint32_t sends = 0; sends != frequencyNonSafety; ++sends) {
				Simulator::Schedule(
						Seconds(rngNonSafety->GetValue(time, time + 1)),
						&MultipleChannelExperiment::SendIpPackets, this,
						sender);
			}
		}
	}
}
void MultipleChannelExperiment::ConfigurationB(void) {
	NS_LOG_FUNCTION(this);
	NetDeviceContainer::Iterator i;
	for (i = devices.Begin(); i != devices.End(); ++i) {
		Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice>(*i);
...
void MultipleChannelExperiment::ConfigurationC(void) {
	NS_LOG_FUNCTION(this);
	...
	Simulator::Schedule(Seconds(0.0), &WaveNetDevice::StartSch, sender,
			schInfo);
	TxProfile profile = TxProfile(SCH1);
	Simulator::Schedule(Seconds(0.0), &WaveNetDevice::RegisterTxProfile, sender,
			profile);
	Ptr<ChannelCoordinator> coordinator = sender->GetChannelCoordinator();
	while (getCurrentTime() - time_start < (double) simulationTime - 1) {

		double time = getCurrentTime() - time_start;
		Time t = Seconds(time);
		std::cout << t.GetMilliSeconds() << std::endl;
		//Time t = Seconds (rngSafety->GetValue (time, time + 1));
		// std::cout<<(coordinator->GetCchInterval() - coordinator->GetIntervalTime(t)).GetMilliSeconds()<<std::endl;

		// if the send time is not at CCHI, we will calculate a new time
		if (coordinator->IsCchInterval(t) && flag == 0) {
			flag = 1;
			std::cout << "time: "
					<< coordinator->GetIntervalTime(t).GetMilliSeconds();
			std::cout << "  is in CCH interval! " << std::endl;
			if (coordinator->IsRMInterval(t)) {
				std::cout << "time: "
						<< coordinator->GetIntervalTime(t).GetMilliSeconds();
				std::cout << "  is in RM interval! " << std::endl;
				for (i = devices.Begin(); i != devices.End(); ++i) {
					Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice>(*i);
					SchInfo schInfo = SchInfo(SCH1, false, 0x0);
					Simulator::Schedule(Seconds(0.0), &WaveNetDevice::StartSch,
							sender, schInfo);
					TxProfile profile = TxProfile(SCH1);
					Simulator::Schedule(Seconds(0.0),
							&WaveNetDevice::RegisterTxProfile, sender, profile);
					Ptr<ChannelCoordinator> coordinator =
							sender->GetChannelCoordinator();
					Simulator::Schedule(t,
							&MultipleChannelExperiment::SendWsmpPackets, this,
							sender, CCH);
				}
			} else if (coordinator->IsCDQInterval(t)) {
				std::cout << "time: "
						<< coordinator->GetIntervalTime(t).GetMilliSeconds();
				std::cout << "  is in CDQ interval! " << std::endl;
			} else if (coordinator->IsAMInterval(t)) {
				std::cout << "time: "
						<< coordinator->GetIntervalTime(t).GetMilliSeconds();
				std::cout << "  is in AM interval! " << std::endl;
			}
		} else if (coordinator->IsSchInterval(t) && flag == 1) {
			flag = 0;
			std::cout << "time: "
					<< coordinator->GetIntervalTime(t).GetMilliSeconds();
			std::cout << "  is in AM interval! " << std::endl;
			Simulator::Schedule(t, &MultipleChannelExperiment::SendIpPackets,
					this, sender);
		}
...
void MultipleChannelExperiment::ConfiguartionD(void) {
	NS_LOG_FUNCTION(this);
	NetDeviceContainer::Iterator i;
	for (i = devices.Begin(); i != devices.End(); ++i) {
		Ptr<WaveNetDevice> sender = DynamicCast<WaveNetDevice>(*i);
		...
		for (uint32_t time = 0; time != simulationTime; ++time) {
			for (uint32_t sends = 0; sends != frequencySafety; ++sends) {
				...
				if (!coordinator->IsSchInterval(t)) {
					t =
							t + coordinator->NeedTimeToSchInterval(t)
									+ MicroSeconds(
											rngOther->GetInteger(0,
													coordinator->GetSchInterval().GetMicroSeconds()
															- 1));
			...
			for (uint32_t sends = 0; sends != frequencyNonSafety; ++sends) {
				...
				if (!coordinator->IsCchInterval(t)) {
					t =
							t + coordinator->NeedTimeToCchInterval(t)
									+ MicroSeconds(
											rngOther->GetInteger(0,
													coordinator->GetCchInterval().GetMicroSeconds()
															- 1));
...
void MultipleChannelExperiment::Run(void) {
	NS_LOG_FUNCTION(this);
...
void MultipleChannelExperiment::Stats(void) {
	NS_LOG_FUNCTION(this);
	...
	NS_LOG_UNCOND(
			"  " << sendSafety.sendInCguardi << " " << sendSafety.sendInCchi << " " << sendSafety.sendInSguardi << " " << sendSafety.sendInSchi);
	...
	NS_LOG_UNCOND("  CGuardI CCHI SGuardI SCHI ");
	NS_LOG_UNCOND(
			"  " << sendNonSafety.sendInCguardi << " " << sendNonSafety.sendInCchi << " " << sendNonSafety.sendInSguardi << " " << sendNonSafety.sendInSchi);
	...
	double nonSafetyPDR = receiveNonSafety
			/ (double) (nonSafetyPacketID - queueNonSafety);
	NS_LOG_UNCOND(
			"  safetyPDR = " << safetyPDR << " , nonSafetyPDR = " << nonSafetyPDR);
	// stats average delay
	double delaySafety = timeSafety / receiveSafety / 1000.0;
	double delayNonSafety = timeNonSafety / receiveNonSafety / 1000.0;
	NS_LOG_UNCOND(
			"  delaySafety = " << delaySafety << "ms" << " , delayNonSafety = " << delayNonSafety << "ms");
	// stats average throughout
	double throughoutSafety = receiveSafety * sizeSafety * 8 / simulationTime
			/ 1000.0;
	double throughoutNonSafety = receiveNonSafety * sizeNonSafety * 8
			/ simulationTime / 1000.0;
	NS_LOG_UNCOND(
			"  throughoutSafety = " << throughoutSafety << "kbps" << " , throughoutNonSafety = " << throughoutNonSafety << "kbps");
}
